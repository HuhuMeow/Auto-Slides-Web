...
from backend.llm.compat import patch_langchain_openai, patch_openai_client

# 导入提示词
from backend.prompts import TEX_GENERATION_PROMPT

# 导入特殊字符处理
from backend.services.latex import clean_caption_for_latex
...
class TexGenerationAgent:
    ...
    def _init_model(self):
        ...
        if not OPENAI_AVAILABLE:
            ...
            self.llm = None
            emit_progress("TEX Generation Agent", "TEX generation dependencies are unavailable", stage="generating_tex", level="error")
            return

        if not self.api_key:
            self.logger.warning("未提供OpenAI API密钥，将无法使用大语言模型生成TEX代码")
            self.llm = None
            emit_progress("TEX Generation Agent", "No model credential is available for TEX generation", stage="generating_tex", level="error")
            return

        try:
            emit_progress("TEX Generation Agent", f"Initializing TEX generation model {self.model_name}", stage="generating_tex", progress=82)
            self.llm = ChatOpenAI(
            ...
            emit_progress("TEX Generation Agent", "TEX generation model is ready", stage="generating_tex", progress=83)
        except Exception as e:
            self.logger.error(f"初始化语言模型失败: {str(e)}")
            self.llm = None
            emit_progress("TEX Generation Agent", "TEX generation model initialization failed", stage="generating_tex", level="error")
    ...
    def generate_tex(self) -> str:
        ...
        emit_progress("TEX Generation Agent", f"Preparing Beamer source for {len(slides_plan)} planned slides", stage="generating_tex", progress=84)

        # 预处理幻灯片中的图片引用
        self._preprocess_slide_figures(slides_plan)
        figure_count = sum(1 for slide in slides_plan if slide.get("includes_figure"))
        emit_progress("TEX Generation Agent", f"Normalized {figure_count} slide figure reference(s)", stage="generating_tex", progress=85)

        language_prompt = (
            "Please generate the Beamer presentation in Simplified Chinese. "
            "All visible slide titles, bullet points, captions, section names, and notes should be Chinese. "
            "Keep LaTeX commands, package names, file paths, equations, code identifiers, paper titles, and author names unchanged when appropriate."
            if self.language == "zh"
            else "Please generate the Beamer presentation in English."
        )
        ...
        try:
            # 调用LLM生成TEX代码
            emit_progress("TEX Generation Agent", "Generating complete theme-aware Beamer source", stage="generating_tex", progress=87)
            ...
            emit_progress("TEX Generation Agent", f"Generated and normalized {len(tex_code):,} characters of TEX", stage="generating_tex", progress=89, level="success")
            ...
            emit_progress("TEX Generation Agent", "TEX source generation failed", stage="generating_tex", level="error")
            return ""
        ...
        for slide in slides_plan:
            if slide.get("includes_figure") and slide.get("figure_reference"):
                ...
                if fig_ref:
                    ...
                    # 检查文件是否存在
                    if "filename" in fig_ref and self.images_dir:
                        img_path = os.path.join(self.images_dir, fig_ref["filename"])
...
精简后的caption:"""
    ...
    def _apply_special_char_handling(self, tex_code: str) -> str:
        ...
        """
        try:
            from backend.services.latex import convert_unicode_to_latex, ensure_latex_packages
